...
#include "descriptors/SerialDescriptors.h"
...
static const uint8_t *getConfigurationDescriptor(uint16_t *size, InputMode mode)
{
	switch (mode)
	{
		...
		case INPUT_MODE_SERIAL:
			*size = sizeof(serial_configuration_descriptor);
...
static const uint8_t *getDeviceDescriptor(uint16_t *size, InputMode mode)
{
	switch (mode)
	{
		...
		case INPUT_MODE_SERIAL:
			*size = sizeof(serial_device_descriptor);
			return serial_device_descriptor;
...
static const uint8_t *getHIDDescriptor(uint16_t *size, InputMode mode)
{
	switch (mode)
	{
		...
		case INPUT_MODE_SERIAL:
			*size = 0; // sizeof(serial_hid_descriptor);
			return NULL; // serial_hid_descriptor;
...
static const uint8_t *getHIDReport(uint16_t *size, InputMode mode)
{
	switch (mode)
	{
		...
		case INPUT_MODE_SERIAL:
			*size = 0; // sizeof(serial_report_descriptor);
			return NULL; // serial_report_descriptor;
...
static const uint16_t *getStringDescriptor(uint16_t *size, InputMode mode, uint8_t index)
{
	...
	else
	{
		switch (mode)
		{
			...
			case INPUT_MODE_SERIAL:
				str = (char *)serial_string_descriptors[index];
				break;
